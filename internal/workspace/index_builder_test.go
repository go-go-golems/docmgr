...
`)
	...
	// Related file should carry the collapsed paths-v2 keys: resolved abs + anchor
	// (+ repo-relative display form).
	var anchor, normAbs, normRepoRel string
...
LIMIT 1
`).Scan(&anchor, &normAbs, &normRepoRel); err != nil {
		t.Fatalf("select related_files normalization: %v", err)
	}
	if anchor != "repo" {
		t.Fatalf("expected anchor=repo, got %q", anchor)
	}
	wantAbs := filepath.ToSlash(filepath.Join(repoRoot, "backend", "main.go"))
	if normAbs != wantAbs {
		t.Fatalf("expected norm_abs=%q, got %q", wantAbs, normAbs)
	}
