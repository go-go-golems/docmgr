...
- [x] Implement template support for search command - refactor to collect results before printing, build template data struct with Query, Results (Path, Title, Ticket, Snippet), TotalResults. Render templates/doc/search.templ after human output.
- [x] Implement template support for tasks list command - build template data struct with TotalTasks, OpenTasks, DoneTasks, Tasks array. Render templates/tasks/list.templ after human output.
- [x] Implement template support for vocab list command - build template data struct with Category filter and vocabulary items by category (Topics, DocTypes, Intent, Status). Render templates/vocab/list.templ after human output.
- [x] Implement template support for guidelines command - build template data struct with DocType, GuidelineText, Source, Path. Render templates/doc/guidelines.templ after human output. (Low priority - mostly static content)
...
- [x] Create example template for status (ttmp/templates/status.templ)
...
- [x] Create example template for vocab list (ttmp/templates/vocab/list.templ)
